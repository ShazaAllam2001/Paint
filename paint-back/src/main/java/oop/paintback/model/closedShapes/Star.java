...
public class Star extends closedShape {
    ...
    public Star() { }
    public Star(double centerX,double centerY,double radius,String color,String id){
    ...
    public void addNewShape(String jsonString, RunningData data) {
        ...
        Star star=new Star(jsonObject.getDouble("centerX"),jsonObject.getDouble("centerY"),jsonObject.getDouble("radius"),jsonObject.getString("color"),jsonObject.getString("id"));
        data.getCircles().add(star);
    }
    ...
    public static Star getInstance(){
        if(instance==null){
            synchronized (Star.class){
                if(instance==null) instance=new Star();
            }
        }
        return instance;
    }
    ...
    public boolean CheckIn(double pointx, double pointy, Object shape) {
        return !(Math.sqrt(Math.pow(pointx - ((Star) shape).getCenterX(), 2)
                + Math.pow(pointy - ((Star) shape).getCenterY(), 2)) > (((Star) shape).getRadius() + 1.5));
    }
