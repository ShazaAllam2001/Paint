...
public class Heptagon extends Polygon {
    ...
    private double point4X, point4Y, point5X, point5Y, point6X, point6Y, point7X, point7Y;

    public Heptagon() {
    }

    public Heptagon(double point1X, double point1Y, double point2X, double point2Y, double point3X, double point3Y,
                    double point4X, double point4Y, double point5X, double point5Y, double point6X, double point6Y,
                    double point7X, double point7Y,
        ...
        this.point7X = point7X; this.point7Y = point7Y;
    ...
    public double getPoint7X() {
        return point7X;
    }
    public double getPoint7Y() {
        return point7Y;
    }
    ...
    public void setPoint7X(double point1X) {
        this.point7X = point1X;
    }
    public void setPoint7Y(double point1Y) {
        this.point7Y = point1Y;
    }
    ...
    public void copy(double selectedX, double selectedY, double pointX, double pointY, int index, RunningData data) {
        Heptagon t;
        t = (Heptagon) data.getHeptagons().get(index).clone();
    ...
    public void addNewShape(String jsonString, RunningData data) {
        ...
        Heptagon heptagon = new Heptagon(jsonObject.getDouble("point1X"), jsonObject.getDouble("point1Y"),
                ...
                jsonObject.getDouble("point7X"), jsonObject.getDouble("point7Y"),
                jsonObject.getString("color"),jsonObject.getString("id"));
        data.getHeptagons().add(heptagon);
    }

    @Override
    public void delete(int index, RunningData data) {
        data.getHeptagons().remove(index);
    }

    @Override
    public void resize(int index, RunningData data, double scalar) {
        Heptagon t = data.getHeptagons().get(index);
        double differenceX = (t.point1X + t.point2X + t.point3X + t.point4X + t.point5X + t.point6X + t.point7X) * (1-Math.sqrt(scalar)) / 3;
        double differenceY = (t.point1Y + t.point2Y + t.point3Y + t.point4Y + t.point5Y + t.point6Y + t.point7Y) * (1-Math.sqrt(scalar)) / 3;
        ...
        t.point7X *= Math.sqrt(scalar)+differenceX;
        t.point7Y *= Math.sqrt(scalar)+differenceY;
        t.setId(Integer.parseInt(data.getShapesStack().peek())+1+"");
    }
    ...
    public void print(RunningData data) {
        for (int i = 0; i < data.getTriangles().size(); i++) {
            ...
            System.out.print(data.getHeptagons().get(i).getPoint7X() + "\t");
            System.out.print(data.getHeptagons().get(i).getPoint7Y() + "\n");
        }
    }

    @Override
    public boolean CheckIn(double pointx, double pointy, Object shape) {
        Heptagon tr = (Heptagon) shape;
    ...
    public org.json.simple.JSONObject dataToString(LoadingData data, int index) {
        ...
        jsonObject.put("point1X",data.getHeptagons().get(index).getPoint1X());
        jsonObject.put("point1Y",data.getHeptagons().get(index).getPoint1Y());
        jsonObject.put("point2X",data.getHeptagons().get(index).getPoint2X());
        jsonObject.put("point2Y",data.getHeptagons().get(index).getPoint2Y());
        jsonObject.put("point3X",data.getHeptagons().get(index).getPoint3X());
        jsonObject.put("point3Y",data.getHeptagons().get(index).getPoint3Y());
        jsonObject.put("point1X",data.getHeptagons().get(index).getPoint1X());
        jsonObject.put("point1Y",data.getHeptagons().get(index).getPoint1Y());
        jsonObject.put("point2X",data.getHeptagons().get(index).getPoint2X());
        jsonObject.put("point2Y",data.getHeptagons().get(index).getPoint2Y());
